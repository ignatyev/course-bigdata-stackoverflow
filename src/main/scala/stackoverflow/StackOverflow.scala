...
object StackOverflow extends StackOverflow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    assert(scored.filter(_ == (Posting(1, 72, None, None, 16, Some("Ruby")), 3)).collect().length != 0)
    val vectors = vectorPostings(scored)
    //    assert(vectors.filter(_ == (350000, 67)).collect().length != 0)
    assert(vectors.count() == 2121822, "Incorrect number of vectors: " + vectors.count())

    val means = kmeans(sampleVectors(vectors), vectors, debug = true)
    val results = clusterResults(means, vectors)
    //    if (kmeansMaxIterations == 1) assert(results.contains(("Ruby", 100, 108, 39)))
...
//    def kmeansMaxIterations = 1
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val clustered: RDD[(Int, Iterable[(Int, Int)])] = vectors.map(p => (findClosest(p, means), p)).groupByKey()
    clustered.map { case (index, vs) =>
      (index, averageVectors(vs))
    }.collect().foreach { case (ind, mean) => newMeans(ind) = mean }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val clusterSize: Int = vs.size
      val medianScore: Int = vs.map(_._2).toArray.sorted.apply(vs.size / 2)
      //    if (kmeansMaxIterations == 1) assert(results.contains(("Ruby", 100, 108, 39)))
